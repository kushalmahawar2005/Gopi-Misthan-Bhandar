...
import { FiInstagram, FiPlus, FiEdit, FiTrash2, FiSearch, FiArrowUp, FiArrowDown } from 'react-icons/fi';
...
export default function InstaPostPage() {
  ...
  const fetchInstaPosts = async () => {
    try {
      const response = await fetch('/api/instapost/all');
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Are you sure you want to delete this InstaPost?')) return;

    try {
      const response = await fetch(`/api/instapost/${id}`, {
        method: 'DELETE',
      });
      const data = await response.json();
      if (data.success) {
  ...
  const updateOrder = async (id: string, direction: 'up' | 'down') => {
    const item = instaPosts.find((ip) => ip._id === id);
    if (!item) return;

    const sortedItems = [...instaPosts].sort((a, b) => a.order - b.order);
    const currentIndex = sortedItems.findIndex((ip) => ip._id === id);
    ...
    if (direction === 'up' && currentIndex > 0) {
      ...
      try {
        await Promise.all([
          fetch(`/api/instapost/${id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ order: newOrder }),
          }),
          fetch(`/api/instapost/${prevItem._id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
      ...
      try {
        await Promise.all([
          ...
          fetch(`/api/instapost/${nextItem._id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
  ...
  const filteredInstaPosts = instaPosts
    .filter((item) => {
      if (!searchTerm) return true;
      const searchLower = searchTerm.toLowerCase();
      const caption = (item.caption || '').toLowerCase();
      const instagramUrl = (item.instagramUrl || '').toLowerCase();
      return caption.includes(searchLower) || instagramUrl.includes(searchLower);
    })
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading InstaPost items...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-4">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search InstaPost items..."
            value={searchTerm}
          ...
          <div key={item._id} className="bg-white border border-gray-200 rounded-lg p-5 hover:shadow-md transition-shadow">
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              <button
                ...
                disabled={index === filteredInstaPosts.length - 1}
                className="p-2 text-primary-red hover:bg-red-50 rounded transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                title="Move down"
              >
                <FiArrowDown size={14} />
      ...
      {filteredInstaPosts.length === 0 && (
        <div className="bg-white rounded-lg border border-gray-200 p-12 text-center">
          <p className="text-gray-500 mb-4">No InstaPost items found</p>
          <Link
            href="/admin/instapost/new"
            className="inline-block text-primary-red hover:underline font-medium"
          >
            Add your first InstaPost item
          </Link>
        </div>
      )}
    </div>
