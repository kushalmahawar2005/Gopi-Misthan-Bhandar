...
export default function EditInstaPost() {
  ...
  const fetchInstaPost = async () => {
    try {
      ...
      const response = await fetch(`/api/instapost/${params.id}?t=${Date.now()}`);
      ...
      console.error('Error fetching InstaPost:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.instagramUrl || !formData.instagramUrl.trim()) {
    ...
    try {
      ...
      console.log('Updating InstaPost with:', updatePayload);
      
      const response = await fetch(`/api/instapost/${params.id}`, {
        method: 'PUT',
      ...
      if (data.success) {
        alert('InstaPost updated successfully!');
        router.push('/admin/instapost');
      } else {
        alert('Error updating InstaPost: ' + (data.error || 'Unknown error'));
      }
    } catch (error) {
      console.error('Error updating InstaPost:', error);
      alert('Error updating InstaPost. Please try again.');
    } finally {
      setSaving(false);
    }
  };
